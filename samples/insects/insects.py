...
class InsectDataset(utils.Dataset):
    ...
    def load_insect(self, dataset_dir, subset):
        ...
        abeille_mellifere_dir = os.path.join(dataset_dir, "abeille_mellifere")
        bourdon_des_arbres_dir = os.path.join(dataset_dir, "bourdon_des_arbres")
        abeille_mellifere_list= [f for f in os.listdir(abeille_mellifere_dir)if os.path.isfile(os.path.join(abeille_mellifere_dir, f))]
        bourdon_des_arbres_list = [f for f in os.listdir(bourdon_des_arbres_dir)if os.path.isfile(os.path.join(bourdon_des_arbres_dir, f))]

        for filename in abeille_mellifere_list:
            image_path = os.path.join(abeille_mellifere_dir, filename)
        ...
        # for a in annotations:
            ...
            # the outline of each insect instance. There are stores in the
            # shape_attributes (see json format above)
            # polygons = [r['shape_attributes'] for r in a['regions']]
            # insects = [s['region_attributes']['insects'] for s in a['regions']]
            # print("insects:", insects)
            # name_dict = {"abeille_mellifere": 1, "bourdon_des_arbres": 2}
            # key = tuple(name_dict)
            # num_ids = [name_dict[a] for a in insects]

            # num_ids = [int(n['Event']) for n in insects]
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train Mask R-CNN to detect insect class.')
